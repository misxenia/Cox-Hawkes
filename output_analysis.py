...
with open(filename+'output.pkl', 'rb') as file:
	output = dill.load(file)
	#model=output['model']
	mcmc_samples=output['samples '+str(i)]
	mcmc=output['mcmc '+str(i)]
...
if save_me:
	output_dict={}
	output_dict['prediction_error_A '+str(i)]=np.mean(ErrorA)
	output_dict['prediction_error_B '+str(i)]=np.mean(ErrorB)
	output_dict['prediction_error_C '+str(i)]=np.mean(ErrorC)
